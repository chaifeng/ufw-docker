...
When the configured agent image ends with `-legacy` or `-nf_tables`, `ufw-docker` will automatically choose the compatible backend at runtime. It prefers `-legacy` on hosts already using legacy iptables; otherwise it probes the nftables variant and falls back to `-legacy` if that probe fails. Manually forcing a specific image with commands such as `docker service update --image ...` bypasses this safeguard.
...
当配置的 agent 镜像标签以 `-legacy` 或 `-nf_tables` 结尾时，`ufw-docker` 会在运行时自动选择兼容的后端。如果宿主机已经在使用 legacy iptables，会直接选择 `-legacy`；否则会先探测 nftables 变体是否可用，探测失败时自动回退到 `-legacy`。如果像 `docker service update --image ...` 这样手动强制指定镜像，则会绕过这层保护。
